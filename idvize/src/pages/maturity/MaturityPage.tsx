...
function ConfidencePill({ value }: { value: number }) {
  ...
  const cls = pct >= 70 ? 'bg-green-500/20 text-a-green' : pct >= 40 ? 'bg-amber-500/20 text-a-amber' : 'bg-red-500/20 text-a-red'
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="flex items-start justify-between gap-2 mb-3">
        <span className="text-sm font-medium text-body leading-tight">{domain.name}</span>
      ...
      <div className="text-xs text-a-indigo opacity-0 group-hover:opacity-100 transition-opacity mt-1">
...
export default function MaturityPage() {
  ...
  if (error) return (
    <div className="p-6 text-a-red text-sm">Error: {error}</div>
  )
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-heading">IAM Program Maturity</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div className="lg:col-span-1 rounded-xl border border-surface-600 bg-surface-800 p-6 flex flex-col items-center">
          ...
          <div className="mt-3 text-center">
            <p className="text-sm text-muted">Overall Programme Maturity</p>
        ...
        <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
            <div className="flex items-center gap-2 mb-2">
              <AlertTriangle size={16} className="text-a-red" />
              <span className="text-sm font-medium text-secondary">Weakest Domains</span>
            </div>
            {weakDomains.map(d => (
              <div key={d.domainId} className="flex items-center justify-between py-1 border-b border-surface-700 last:border-0">
                ...
                <span className="text-xs font-semibold text-a-red ml-2">{d.score}</span>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-4">
            <div className="flex items-center gap-2 mb-2">
              <CheckCircle size={16} className="text-a-green" />
              <span className="text-sm font-medium text-secondary">Strongest Domains</span>
            </div>
            {strongDomains.map(d => (
              <div key={d.domainId} className="flex items-center justify-between py-1 border-b border-surface-700 last:border-0">
                <span className="text-xs text-muted truncate">{d.name}</span>
                <span className="text-xs font-semibold text-a-green ml-2">{d.score}</span>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
            <div className="flex items-center gap-2 mb-2">
              <Info size={16} className="text-a-amber" />
              <span className="text-sm font-medium text-secondary">Low Confidence</span>
            </div>
            {lowConfDomains.length === 0
              ? <p className="text-xs text-muted">All domains have acceptable confidence</p>
              : lowConfDomains.map(d => (
                <div key={d.domainId} className="flex items-center justify-between py-1 border-b border-surface-700 last:border-0">
                  <span className="text-xs text-muted truncate">{d.name}</span>
                  <span className="text-xs font-semibold text-a-amber ml-2">{Math.round(d.confidence * 100)}%</span>
                </div>
              ))
            }
          </div>
      ...
      {summary.aiNarrative && (
        <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-5">
          <div className="flex items-center gap-2 mb-3">
            <TrendingUp size={16} className="text-a-indigo" />
            <span className="text-sm font-semibold text-a-indigo">AI Executive Narrative</span>
            <span className="text-xs text-muted ml-2">Powered by Claude — evidence-grounded analysis</span>
          </div>
          <p className="text-sm text-secondary leading-relaxed whitespace-pre-line">{summary.aiNarrative}</p>
        </div>
      )}

      {/* Domain Cards */}
      <div>
        <h2 className="text-base font-semibold text-body mb-3">Domain Scores</h2>
      ...
      {/* Top Recommendations */}
      <div>
        <h2 className="text-base font-semibold text-body mb-3">Top Recommendations</h2>
            ...
            <div key={i} className="rounded-lg border border-surface-600 bg-surface-800 p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <span className="text-sm font-medium text-body">{rec.title}</span>
                  <span className="text-xs px-2 py-0.5 rounded-full font-medium"
                    style={{ backgroundColor: PRIORITY_COLOR[rec.priority] + '25', color: PRIORITY_COLOR[rec.priority] }}>
                    {rec.priority}
                  </span>
                  <span className="text-xs text-muted">{EFFORT_LABEL[rec.effort as keyof typeof EFFORT_LABEL] ?? rec.effort}</span>
                  <span className="text-xs text-a-indigo ml-auto">+{rec.impact} pts impact</span>
                </div>
                <p className="text-xs text-muted">{rec.description}</p>
                <p className="text-xs text-muted mt-1">Domain: {rec.domainName}</p>
              </div>
            </div>
          ))}
        </div>
