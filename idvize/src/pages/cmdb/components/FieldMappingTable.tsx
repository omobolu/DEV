      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={autoFillAll}
            className="text-xs text-violet-400 hover:text-violet-300 px-2 py-1 rounded border border-violet-800/50 hover:bg-violet-900/20 transition-colors">
            Auto-suggest all
          </button>
          <button onClick={clearAll}
            className="text-xs text-slate-500 hover:text-slate-300 px-2 py-1 rounded border border-surface-600 hover:bg-surface-700 transition-colors">
            Clear all
          </button>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-surface-900 border-b border-surface-700">
              <th className="text-left px-4 py-3 text-xs text-slate-400 uppercase tracking-wide w-1/3">Source Column</th>
              <th className="text-left px-4 py-3 text-xs text-slate-400 uppercase tracking-wide w-1/3">Map To</th>
              <th className="text-left px-4 py-3 text-xs text-slate-400 uppercase tracking-wide w-1/3">Sample Value</th>
            </tr>
          </thead>
          <tbody>
            {mappings.map((m, i) => (
              <tr key={m.sourceHeader}
                ...
                <td className="px-4 py-2.5">
                  <span className="font-mono text-xs text-slate-200 bg-surface-700 px-1.5 py-0.5 rounded">
                    {m.sourceHeader}
                  </span>
                </td>
                <td className="px-4 py-2.5">
                  <select
                    ...
                    className="w-full bg-surface-900 border border-surface-600 rounded px-2 py-1 text-xs text-slate-300
