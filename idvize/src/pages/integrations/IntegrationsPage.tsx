...
function StatusBadge({ status }: { status: Status }) {
  const map: Record<Status, { label: string; cls: string }> = {
    connected:     { label: 'Connected',     cls: 'bg-green-500/20 text-green-400 border-green-500/30' },
    failed:        { label: 'Failed',        cls: 'bg-red-500/20 text-red-400 border-red-500/30' },
    not_configured:{ label: 'Not Configured', cls: 'bg-slate-500/20 text-slate-400 border-slate-500/30' },
    mock:          { label: 'Mock Mode',     cls: 'bg-amber-500/20 text-amber-400 border-amber-500/30' },
  }
...
export default function IntegrationsPage() {
  ...
  const renderForm = () => {
    ...
    const inp = 'w-full bg-surface-900 border border-surface-600 rounded-lg px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500'
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-slate-100">IAM Platform Integrations</h1>
        <p className="text-sm text-slate-400 mt-1">Connect idvize to your IAM tools. Credentials are saved to the API server's .env file.</p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {PLATFORMS.map(p => (
          ...
          >
            ...
            <div className="text-sm font-medium text-slate-200 leading-tight">{p.label}</div>
            <div className="mt-2">
      ...
      <div className="rounded-xl border border-surface-600 bg-surface-800 overflow-hidden">
        <div className="px-6 py-4 border-b border-surface-700 flex items-center justify-between">
          <div>
            <h2 className="text-base font-semibold text-slate-100">{activePlatform.logo} {activePlatform.label}</h2>
            <p className="text-xs text-slate-400 mt-0.5">{activePlatform.description}</p>
          </div>
