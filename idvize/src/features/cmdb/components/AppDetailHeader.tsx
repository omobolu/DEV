    ...
    <div className="flex items-start justify-between gap-4">
      <div className="flex items-start gap-4">
        <button
          ...
          className="mt-0.5 p-1.5 rounded-lg text-muted hover:text-body hover:bg-surface-700 transition-colors flex-shrink-0"
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="text-xs font-mono text-muted">{appId}</span>
          </div>
          <h1 className="text-2xl font-bold text-heading leading-tight">{name}</h1>
          <div className="flex flex-wrap items-center gap-2 mt-2">
            {tags.map(tag => (
              <span
                key={tag}
                className="text-xs text-muted bg-surface-700 border border-surface-600 px-2 py-0.5 rounded-full"
              >
                {tag}
              </span>
            ))}
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <div className="text-right">
          <p className="text-xs text-muted mb-1">Criticality</p>
