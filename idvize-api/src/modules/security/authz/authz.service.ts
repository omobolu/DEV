...
class AuthzService {
  ...
  check(userId: string, permission: PermissionId, context: EvaluationContext = {}, tenantId?: string): AuthzDecision {
    const user = tenantId
      ? authRepository.findById(tenantId, userId)
      : authRepository.findByIdGlobal(userId);
    if (!user) {
