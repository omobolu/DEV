...
- `<name>.repository.ts` — In-memory Map storage, partitioned by tenantId
...
- Repositories: `Map<tenantId, Map<entityId, Entity>>` — always scope by tenant
- Seed data: lazy-loaded via `ensureSeeded(tenantId)` pattern in services
